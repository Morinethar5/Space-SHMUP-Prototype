...
public class Utils : MonoBehaviour {
	...
	public static Bounds BoundsUnion (Bounds b0, Bounds b1) {
		//If the size of one the bounds is Vector3.zero, ignore that one
	...
	//Checks to see whether the bounds bnd are within the cambound
	...
	public static Vector3 BoundsInBoundsCheck (Bounds bigB, Bounds lilB, BoundsTest test = BoundsTest.onScreen) {
		//The behaviour of this function is different based on the BoundsTest that been selected
		...
		case BoundsTest.onScreen:
			...
			} else if (lilB.min.y < bigB.max.y) {
				off.y = lilB.min.y - bigB.min.y;
			}
			if (lilB.max.z > bigB.max.z) {
				off.z = lilB.max.z - bigB.max.z;
			} else if (lilB.min.z < bigB.max.z) {
	...
	//this function will iteratively climb up the transform.parent tree untill it either finds a prarent with a tag != "untagged" or no parent
	public static GameObject FindTaggedParent(GameObject go) {
		//if this gameojbect has a tag:
		if (go.tag != "Untagged") {
			//then return this
			return(go);
		}
